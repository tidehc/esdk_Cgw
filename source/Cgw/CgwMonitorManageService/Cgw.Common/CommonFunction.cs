...
namespace CgwMonitorManage.Common
{
    public class CommonFunction
    {
        ...
        /// 字符串转成字节，加密
        /// </summary>
        /// <param name="pwd">字符串加密</param>
        /// <param name="nodeEntropy">增加机密效果，节点名称</param>
        /// <returns></returns>
        public static byte[] EncryptStr2Byte(string pwd,string nodeEntropy)
        {
            byte[] pwdByte = Encoding.Default.GetBytes(pwd);
            //pwdByte = ProtectedData.Protect(pwdByte, null, DataProtectionScope.CurrentUser);
            pwdByte = ProtectedData.Protect(pwdByte, Encoding.Default.GetBytes(nodeEntropy), DataProtectionScope.CurrentUser);
            return pwdByte;
        }
        ...
        /// 解密，字节转成字符串
        /// </summary>
        /// <param name="pwd">密码数组解密</param>
        /// <param name="nodeEntropy">增加机密效果，节点名称</param>
        /// <returns></returns>
        public static string DecryptByte2Str(byte[] pwd,string nodeEntropy)
        {
            //byte[] pwdByte = ProtectedData.Unprotect(pwd, null, DataProtectionScope.CurrentUser);
            byte[] pwdByte = ProtectedData.Unprotect(pwd, Encoding.Default.GetBytes(nodeEntropy), DataProtectionScope.CurrentUser);
            string pwdStr = Encoding.Default.GetString(pwdByte);
            return pwdStr;
        }
    }
}
